...
in
{
  options.programs.tmux = {
    captureDirectory = lib.mkOption {
      type = lib.types.str;
      default = "~/screenshots";
    };
    captureFilename = lib.mkOption {
      type = lib.types.str;
      default = "#{t/f/%Y-%m-%dT%H-%M-%S%z:window_activity}.txt";
    };
  };
  ...
  config.programs.tmux =
    let
      ...
      capturePath = "${cfg.captureDirectory}/${cfg.captureFilename}";
    in
    {
      enable = true;
      escapeTime = 0;
